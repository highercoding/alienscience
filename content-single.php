...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		...
		<div class="entry-meta">
			<?php if(!in_category(1)) alienscience_posted_on(); ?>
		</div><!-- .entry-meta -->
	</header><!-- .entry-header -->
	...
	<footer class="entry-meta">
		<?php
			...
			$category_list = get_the_category_list( __( ', ', 'alienscience' ) );

			/* translators: used between list items, there is a space after the comma */
			$tag_list = get_the_tag_list( '', ', ' );

			if ( ! alienscience_categorized_blog() ) {
				// This blog only has 1 category so we just need to worry about tags in the meta text
				if ( '' != $tag_list ) {
					$meta_text = __( 'Tagged %2$s.', 'alienscience' );
				}

			} else {
				// But this blog has loads of categories so we should probably display them here
				if ( '' != $tag_list ) {
					$meta_text = __( 'Posted in %1$s and tagged %2$s.', 'alienscience' );
				}
